...
class FavouriteItemWidget extends StatefulWidget {
  const FavouriteItemWidget({super.key, required this.project, this.showLikeButton = false});
  final FavouriteProjectSearchItem project;
...
class _FavouriteItemWidgetState extends State<FavouriteItemWidget> {
        ...
        children: [
          ...
          if (widget.showLikeButton)
            Positioned(
                ...
                right: 0,
                child: LikeButton(
                  value: widget.project.isFavorite,
  ...
  Future<void> likeChanged(BuildContext context, bool value) async {
    await context.read<ProjectProvider>().setFavorite(widget.project.id, value);
    setState(() {
      widget.project.isFavorite = value;
    });
  }
}
